...
class Train extends JFrame implements ActionListener{
	...
	Train(){
		...
		comboBox_name.addItem("KTX");
		comboBox_name.addItem("무궁화 호");
		comboBox_name.addItem("SRT");
		comboBox_name.addItem("ITX-청춘");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==button)
		{
			...
			this.date = comboBox_year.getSelectedItem().toString() +" 년"
					+comboBox_month.getSelectedItem().toString()+" 월"
					+comboBox_day.getSelectedItem().toString()+ " 일"
					+comboBox_hour.getSelectedItem().toString()+" 시"
					+comboBox_minute.getSelectedItem().toString()+ " 분";
